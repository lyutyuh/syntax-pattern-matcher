...
def _skipgrams(sequence, n, k, pad_left=False, pad_right=False, pad_symbol=None):
    ...
    # If n+k longer than sequence, reduce k by 1 and recur
        ...
        for ng in _skipgrams(list(sequence), n, k-1):
            yield tuple(ng)
    ...
    # len(sequence) < n+k
    for ng in list(_skipgrams(history, n, k-1)):
        yield tuple(ng)

def skipgrams(sequence, n, k):
    _seq = list(zip(range(len(sequence)), sequence))
    return list(_skipgrams(_seq, n, k))
